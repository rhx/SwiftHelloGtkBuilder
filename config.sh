...
MACOSX_RESOURCES_DIR=$PWD/Resources/macOS
RESOURCES_DIR=$PWD/Sources/$Mod/Resources
BUNDLE_NAME="${Mod}_${Mod}.bundle"
if [ -z "$BUILD_DIR" ]; then
...
LINKFLAGS="-Xlinker -lgtk-3 "`pkg-config --libs gtk+-$ver gdk-$ver pangocairo pangoft2 pango gobject-2.0 gio-unix-2.0 glib-2.0 | sed -e 's/ *--export-dynamic */ /g' -e 's/ *-Wl, */ /g' -e 's/,/ -Xlinker /g' -e 's/-pthread/-lpthread/g' -e 's/  */ /g' -e 's/^ *//' -e 's/ *$//' | tr ' ' '\n' | tr '	' '\n' | sed -e 's/^/-Xlinker /' | tr '\n' ' ' | sed -e 's/-Xlinker *-Xlinker/-Xlinker/g' -e 's/-Xlinker[ 	]*-Xlinker/-Xlinker/g' -e 's/-Xlinker *$//'`" -Xlinker -L/usr/local/lib"
